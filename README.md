...
Repo bài lab **Multi-Agent Systems**: hệ thống nghiên cứu gồm **Supervisor + Researcher + Analyst + Writer** và benchmark với single-agent baseline.

Hệ thống chạy offline bằng deterministic mock khi chưa có API key, hoặc dùng Groq/OpenAI và Tavily khi các key tương ứng được cấu hình.
...
Ở chế độ `auto`, thứ tự chọn provider là Groq → OpenAI → offline mock. Có thể ép provider
bằng `LLM_PROVIDER=groq`, `openai` hoặc `mock`. Groq sử dụng endpoint tương thích OpenAI
...
Trên Windows không có `make` mặc định, chạy trực tiếp:
...
Lệnh này chạy single-agent baseline và ghi JSON trace vào `reports/`.
...
Lệnh chạy workflow có routing, retry, timeout, validation, fallback và JSON trace.

Tạo báo cáo so sánh tại `reports/benchmark_report.md`:
...
1. OpenAI client với retry và offline mock.
